...
// Add a global variable to store all Pokémon names for the dropdown
let allPokemonNames = [];
...
// Fetch all Pokémon names for the dropdown on page load
...
function renderTypeFilter() {
  ...
  if (!filter) {
    ...
    searchInput.parentNode.insertBefore(filter, searchInput);
  ...
  filter.innerHTML = '<option value="">All Types</option>' +
    pokemonTypes.map(type => `<option value="${type}" ${selectedTypeFilter===type?'selected':''}>${type.charAt(0).toUpperCase() + type.slice(1)}</option>`).join('');
}

// Listen for changes on the type filter and update the dropdown
...
// Initial fetch of all Pokémon names
